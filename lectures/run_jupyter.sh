...
isRunning=$(docker ps -f name="^/${CONTAINER_NAME}$" | grep -c "${CONTAINER_NAME}" || true)
...
if [ "$isRunning" -eq 0 ]; then
    ...
    # Start container and run Jupyter in /home/introduction_to_robotics
    docker run \
        -it \
        --name "${CONTAINER_NAME}" \
        ...
        --volume "${ROOT_DIR}/lecture5:/home/introduction_to_robotics/lecture5" \
        "${IMAGE_NAME}" \
        bash -lc 'cd /home/introduction_to_robotics && exec /home/.base/bin/jupyter notebook --allow-root --ip=0.0.0.0 --no-browser --NotebookApp.token='' --NotebookApp.password='''
else
